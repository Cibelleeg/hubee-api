...
export class UsuariosService {
  ...
  async create(data: CreateUsuarioDto) {
    ...
    const [newUser] = await this.db.insert(usuarios).values({
      ...data, 
  ...
  async findOne(id: string) {
    const [user] = await this.db.select().from(usuarios).where(
      and(eq(usuarios.id, id), isNull(usuarios.deletedAt))
    );
  ...
  async update(id: string, data: UpdateUsuarioDto) {
    ...
    if (data.dataNascimento) {
      updateData.dataNascimento = new Date(data.dataNascimento).toISOString().split('T')[0];
    }
    const [updatedUser] = await this.db.update(usuarios)
      .set(updateData)
